...
def make_new_row(old_row, new_question, new_answer):
    new_row = old_row.copy()
    new_row[4] = new_question
    new_row[5] = new_answer
    new_row[8] = 1
    return new_row
...
if __name__ == "__main__":
    ...
    cnt = 0
    re_cnt,pseudo_cnt,use_title_cnt = 0,0,0
    for row in data:
        cnt += 1
        ...
        if q_a.isdigit() and int(q_a) == 1:
            if question.find("温馨提示") != -1 and question_list != []:     # "温馨提示"类型，则在统一content中找前一问题进行拼接
                ...
                if last_align.find("常见") == -1:   # 非"常见问题"类型，拼接align最后一组
                    ...
                    # qa_list.append({"question":new_question, "answer":answer}) 
                    new_row = make_new_row(row,new_question,answer)
    ...
    print(f"no qa data row :{re_cnt+pseudo_cnt+use_title_cnt}, re: {re_cnt}, pseudo: {pseudo_cnt}, title: {use_title_cnt}")
